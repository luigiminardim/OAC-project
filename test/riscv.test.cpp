...
TEST_CASE("load_mem")
{
  SECTION("A função deve carregar as instruções corretamente na memória.")
  {
    ...
    REQUIRE(mem[0] == 0x00002297); // primeira instrução;
    REQUIRE(mem[1] == 0x00028293); // segunda instrução;
  }
  SECTION("A função deve carregar os dados corretamente na memória.")
  {
    load_mem("assets/data.bin", DATA_SEGMENT_START);
    REQUIRE(mem[DATA_SEGMENT_START / 4] == 0x00000001);     // primeiros 4 bytes em data;
    REQUIRE(mem[DATA_SEGMENT_START / 4 + 1] == 0x00000003); // segundos 4 bytes em data;
  }
}
